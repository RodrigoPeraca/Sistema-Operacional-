...
/**
 * Benchmark científico: WorstFitUnsafe vs WorstFitSynchronized vs WorstFitPartitioned.
 *
 * ═══════════════════════════════════════════════════════════════════════════════
 * METODOLOGIA
 * ═══════════════════════════════════════════════════════════════════════════════
 ...
 *    na medição. AtomicInteger é a primitiva correta para contadores compartilhados
 *    de baixa contenção.
 *
 ...
 *    SOLUÇÃO: cada thread mantém seus próprios contadores locais (int[]). Ao final,
 ...
 *    Combina com afinidade por segmento: thread-i processa bloco-i e aloca em seg-i.
 *
 ...
 *   AtomicInteger para fila e contadores de benchmark — NÃO são parte do algoritmo
 */
public class HeapBenchmark {
    ...
    public static final class BenchmarkParams {
        ...
        public BenchmarkParams(int heapKb, int totalRequests, int minBytes, int maxBytes) {
            this.heapKb = heapKb; this.totalRequests = totalRequests;
            this.minBytes = minBytes; this.maxBytes = maxBytes;
        }
    }
    ...
    public static final class BenchmarkResult {
        ...
        BenchmarkResult(BenchmarkParams p, long[] seq, long[] par,
            ...
            this.heapKb = p.heapKb; this.totalRequests = p.totalRequests;
            this.minBytes = p.minBytes; this.maxBytes = p.maxBytes;
            this.threadCount = SYNC_THREAD_COUNT;
            this.warmupRounds = WARMUP_ROUNDS; this.measureRounds = MEASURE_ROUNDS;
            seqLatencyMinMs = seq[0]/1e6; seqLatencyAvgMs = seq[2]/1e6; seqLatencyMaxMs = seq[1]/1e6;
            parLatencyMinMs = par[0]/1e6; parLatencyAvgMs = par[2]/1e6; parLatencyMaxMs = par[1]/1e6;
    ...
    static final class RoundResult {
        ...
        RoundResult(long ns, int sv, int rj, int rn) {
            wallClockNs = ns; served = sv; rejected = rj; randoms = rn;
        }
    ...
    public static BenchmarkResult executarComparativo(BenchmarkParams params) {
        ...
        RoundResult[] seqR = runWithWarmup(r -> runSequentialUnsafe(r, params), reqs);
        RoundResult[] parR = runWithWarmup(r -> runParallelSynchronized(r, params), reqs);
        return new BenchmarkResult(params, aggregate(seqR), aggregate(parR),
            seqR[seqR.length-1], parR[parR.length-1]);
    }
    ...
    static RoundResult runParallelSynchronized(Requisitor_Memoria[] requests, BenchmarkParams p) {
        ...
        int served = 0, rejected = 0, randoms = 0;
        for (int[] c : localCounters) { served += c[0]; rejected += c[1]; randoms += c[2]; }
        return new RoundResult(elapsed, served, rejected, randoms);
    }
    ...
    // =========================================================================
    // VERSÃO 3 — Paralelo com WorstFitPartitioned (N mutexes, afinidade por segmento)
    // =========================================================================
     ...
     * 3. CONTADORES LOCAIS — mesmo esquema do Synchronized: sem mutex, sem contenção.
     *
    ...
    static void printResults(long[] uAgg, RoundResult uLast,
        ...
        // Comparação 1: overhead de sincronização (mesma semântica, 1 thread vs N threads)
        ...
        // Comparação 2: ganho do particionamento (COMPARAÇÃO VÁLIDA — mesmo N de threads)
        ...
        // Referência: Unsafe vs Partitioned (threads diferentes — não usar como speedup)
        ...
        System.out.printf("║ Eficiência paralela (Part): %5.1f%%   %-26s║%n",
            effPart * 100, effPart >= 0.7 ? "(boa — ≥70%)" : effPart >= 0.4 ? "(razoável)" : "(baixa — contenção alta)");
    ...
    public static void main(String[] args) {
        ...
        int heapKb        = 64;
        if (args.length >= 1) {
            try { totalRequests = Integer.parseInt(args[0]); }
            catch (NumberFormatException e) { System.err.println("totalRequests inválido"); System.exit(1); }
        }
        if (args.length >= 2) {
            try { heapKb = Integer.parseInt(args[1]); }
            catch (NumberFormatException e) { System.err.println("heapKb inválido"); System.exit(1); }
        }
