...
import { Play, Pause, RotateCcw, Settings, Workflow } from "lucide-react";
  ...
  return (
    <aside className="w-full border-b border-zinc-800 bg-zinc-950 p-5 lg:h-[calc(100vh-73px)] lg:w-80 lg:border-b-0 lg:border-r">
      <div className="mb-5 flex items-center gap-2">
        ...
        <h2 className="font-semibold text-zinc-100">Configuração</h2>
      </div>

      <div className="space-y-4">
        <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-4 text-xs leading-relaxed text-cyan-100">
          ...
          <p className="text-cyan-100/80">
            Esta tela executa a simulação passo a passo no backend Java,
            mostrando a heap, os logs e as métricas da execução visual.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Mín. bytes">
            <input
              ...
              disabled={isRunning}
              onChange={(e) => updateNumberField("minBytes", e.target.value)}
              className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-cyan-500"
            />
          </Field>
          ...
          <Field label="Máx. bytes">
            <input
              ...
              disabled={isRunning}
              onChange={(e) => updateNumberField("maxBytes", e.target.value)}
              className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-cyan-500"
            />
          </Field>
        </div>
        ...
        <Field label="Total de requisições">
          <input
            ...
            disabled={isRunning}
            onChange={(e) => updateNumberField("totalRequests", e.target.value)}
            className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-cyan-500"
          />
        </Field>
        ...
        {showModeSelector && config.mode === "parallel" && (
          <Field label="Threads simuladas">
            <input
              ...
              disabled={isRunning}
              onChange={(e) => updateNumberField("threads", e.target.value)}
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4 text-xs leading-relaxed text-zinc-400">
          <p className="mb-2 font-semibold text-zinc-300">Nota técnica</p>
          <p>
            Cada célula visual representa uma posição do vetor de inteiros da
            heap. Como cada inteiro equivale a 4 bytes, uma variável de 16 bytes
            ocupa 4 células.
          </p>
        </div>
      </div>
